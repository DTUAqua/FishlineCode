...
namespace Babelfisk.OtolithFileService
{
    ...
    public class FileData
    {
        ...
        /// object wraps.</param>
        internal FileData(string dir, WIN32_FIND_DATA findData, bool directory)
        {
    ...
    public static class FastDirectoryEnumerator
    {
        ...
        public static IEnumerable<FileData> EnumerateDirectories(string path, SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            if (path == null)
            {
                throw new ArgumentNullException("path");
            ...
            return new DirectoryEnumerable(fullPath, false, "*", searchOption);
        }


        public static IEnumerable<FileData> EnumerateFilesAndDirectories(string path, SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            if (path == null)
            {
                throw new ArgumentNullException("path");
            ...
            return new DirectoryEnumerable(fullPath, true, "*", searchOption);
        }
        ...
        /// <see cref="T:System.Collections.Generic.IEnumerable`1"/> interface
        /// </summary>
        private class DirectoryEnumerable : IEnumerable<FileData>
        {
            private readonly string m_path;
            private readonly string m_filter;
            private readonly SearchOption m_searchOption;
            private bool _iterateFilesAndFolders = false;

            /// <summary>
            /// Initializes a new instance of the <see cref="DirectoryEnumerable"/> class.
            /// </summary>
            /// <param name="path">The path to search.</param>
            /// <param name="filter">The search string to match against files in the path.</param>
            /// <param name="searchOption">
            ...
            /// operation should include all subdirectories or only the current directory.
            /// </param>
            public DirectoryEnumerable(string path, bool iterateFilesAndFolders, string filter = "*", SearchOption searchOption = SearchOption.TopDirectoryOnly)
            {
                m_path = path;
                m_filter = filter;
                m_searchOption = searchOption;
                _iterateFilesAndFolders = iterateFilesAndFolders;
            }
            ...
            public IEnumerator<FileData> GetEnumerator()
            {
                return new DirectoryEnumerator(m_path, m_filter, m_searchOption, _iterateFilesAndFolders);
            }
        ...
        private class FileEnumerator : IEnumerator<FileData>
        {
            ...
            {
                get { return new FileData(m_path, m_win_find_data, false); }
            }
        ...
        /// </summary>
        [System.Security.SuppressUnmanagedCodeSecurity]
        private class DirectoryEnumerator : IEnumerator<FileData>
        {
            [DllImport("kernel32.dll", CharSet = CharSet.Auto, SetLastError = true)]
            private static extern SafeFindHandle FindFirstFile(string fileName,
                [In, Out] WIN32_FIND_DATA data);
            ...
            /// operation should include all subdirectories or only the current directory.
            /// </param>
            public DirectoryEnumerator(string path, string filter, SearchOption searchOption, bool iterateFilesAndFolders)
            {
                m_path = path;
                m_filter = filter;
                m_searchOption = searchOption;
            ...
            public FileData Current
            {
                get { return new FileData(m_path, m_win_find_data, true); }
            }
            ...
            public bool MoveNext()
            {
                ...
                while (cont)
                {
                    ...
                    if (retval)
                    {
                        if ((!_iterateFilesAndFolders && !((FileAttributes)m_win_find_data.dwFileAttributes).HasFlag(FileAttributes.Directory)) || m_win_find_data.dwFileAttributes.HasFlag(FileAttributes.ReparsePoint) || m_win_find_data.cFileName == "." || m_win_find_data.cFileName == "..")
                        {
                            //Ignore folders for now.   We call MoveNext recursively here to 
                            // move to the next item that FindNextFile will return.
                            // return MoveNext();
