...
namespace FishLineMeasure.BusinessLogic
{
    public class LookupDataVersioning
    {
        ...
        private void HandleLookupIncludes(string strLookupType, ref List<ILookupEntity> lst, params string[] includes)
        {
            ...
            {
                if (!strIncludeProperty.StartsWith("L_"))
                    continue;
                ...
                var prop = objType.GetField(OfflineMapIncludeField(strLookupType, strCodeColumn), BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.IgnoreCase);
        ...
        private void LoadLocalVersions()
        {
            ...
            if (_dicLocalLookups == null)
            {
                ...
                //Add local lookups
                //Ovigorous - Rogn
                List<ILookupEntity> lstOvigorous = new List<ILookupEntity>()
                {
                    new L_Ovigorous() { OvigorousCode = "Ja", Description = "Med rogn" },
                    new L_Ovigorous() { OvigorousCode = "Nej", Description = "Uden rogn" },
                };
                _dicLocalLookups.Add(typeof(L_Ovigorous).Name, lstOvigorous);
            }
        ...
        {
            ServiceLookupClient ser = null;

            try
            {
                ser = new ServiceLookupClient();

                ser.SupplyCredentials();

                _serverVersions = (ser as FishLineService.ILookup).GetLookupVersions();
            ...
            catch(Exception e)
            {
                if(ser != null)
                    ser.Abort();
                //Don't log error, since this could be happening a lot.
            }
        }
